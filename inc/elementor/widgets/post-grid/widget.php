<?php
namespace Devise_Elementor\Widgets\PostGrid;
...
/**
 * Elementor widget for PostGrid.
 */
class Devise_PostGrid extends Widget_Base {
	...
	public function __construct( $data = [], $args = null ) {
		...
		if ( ! defined( 'DEVISE_ELEMENTOR_WIDGET_POST_GRID_DIR' ) ) {
			define( 'DEVISE_ELEMENTOR_WIDGET_POST_GRID_DIR', rtrim( __DIR__, ' /\\' ) );
		}

		if ( ! defined( 'DEVISE_ELEMENTOR_WIDGET_POST_GRID_URL' ) ) {
			define( 'DEVISE_ELEMENTOR_WIDGET_POST_GRID_URL', rtrim( plugin_dir_url( __FILE__ ), ' /\\' ) );
		}
		wp_register_style( 'devise-post-grid', DEVISE_ELEMENTOR_WIDGET_POST_GRID_URL . '/assets/css/devise-post-grid.min.css', array(), null );
	}
	...
	public function get_title() {
		return __( 'Post Grid', 'web-devise-ebptw' );
	}
	...
	protected function post_title_styles( $control ) {
		...
		$control->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .post-grid .entry-title',
		...
		$control->add_responsive_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .post-grid .entry-title' => 'color: {{VALUE}};',
				],
				'default' => '#0f172a',
			]
		);
	...
	protected function author_styles( $control ) {
		...
		$control->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .post-grid .post-grid-item-author-name a',
				'scheme' => Schemes\Typography::TYPOGRAPHY_1,
				'fields_options' => [
					'typography' => [
						'default' => 'custom',
		...
		$control->add_responsive_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .post-grid .post-grid-item-author-name a' => 'color: {{VALUE}};',
				],
				'default' => '#0f172a',
			]
		);
	...
	protected function date_styles( $control ) {
		...
		$control->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .post-grid .post-grid-item-date .entry-date',
				'scheme' => Schemes\Typography::TYPOGRAPHY_1,
				'fields_options' => [
					'typography' => [
						'default' => 'custom',
		...
		$control->add_responsive_control(
			...
			[
				...
				'label_block' => false,
				'selectors' => [
					'{{WRAPPER}} .post-grid .post-grid-item-date .entry-date' => 'color: {{VALUE}};',
				],
				'default' => '#334155',
			]
		);
	...
	public function render() {
		...
		$this->add_render_attribute( 'post_wrapper', 'class', [ 'post-grid clearfix row' ]);
		...
		if ( $posts->have_posts() ) {
			...
			$preset_path = __DIR__ . '/templates/output-post-grid.php'; ?>
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new Devise_PostGrid() );
